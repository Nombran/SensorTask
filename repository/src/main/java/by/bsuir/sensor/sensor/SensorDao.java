...
public class SensorDao {
    ...
    private CriteriaQuery<Sensor> prepareSensorQuery(SensorParamWrapper params) {
        ...
        if (textPart != null && !textPart.isEmpty()) {
            ...
            Predicate predicateForRangeFrom = cb.like(root.get(Sensor_.RANGE_FROM).as(String.class), "%" + textPart + "%");
            Predicate predicateForRangeTo = cb.like(root.get(Sensor_.RANGE_TO).as(String.class), "%" + textPart + "%");
            Predicate predicateForType = cb.like(cb.lower(root.get(Sensor_.UNIT).get(Unit_.UNIT_NAME)), "%" + textPart + "%");
            Predicate predicateForUnit = cb.like(cb.lower(root.get(Sensor_.SENSOR_TYPE).get(SensorType_.TYPE)), "%" + textPart + "%");
